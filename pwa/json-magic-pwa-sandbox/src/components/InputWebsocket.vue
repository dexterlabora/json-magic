  ...
  <v-card>
    ...
    <v-card-text>
      ...
      <v-card color="grey darken-3">
        <v-card-text>
          <v-text-field label="URL Path" v-model="wsForm.url" class="ml-2"></v-text-field>
          <v-checkbox v-model="wsForm.enabled" label="enabled"></v-checkbox>
        </v-card-text>
      </v-card>
      <v-spacer></v-spacer>
      <v-spacer></v-spacer>
      <v-icon :color="isConnected ? 'green': 'red'" class="mr-2">mdi-cloud</v-icon>
    ...
    <v-card-actions>
      ...
      <v-spacer></v-spacer>
      <v-btn @click="onUpdateWebsocket" color="green">
